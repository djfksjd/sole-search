...
두 조사의 hash_version이 다르면(예: v1↔v2 — 산식이 바뀌어 비교 불가) NEEDS_REHASH가 아니라
**1회 CHANGED(상세 재검증)**로 분류한다 — 재수집해도 old가 구버전이라 영구 루프가 되기 때문.
GONE = 직전에 있었는데 새 조사에 없음 — --out이 아닌 별도 gone_*.jsonl에 기록된다(기회 소멸 알림 재료).
